...
const MessengerTable = ({ title = {}, columns = {}, data = {}, actions = [], components = {}, onRowClick }) => {
  const classes = useStyles();
  const request = useSelector((state) => state.messenger);
  ...
  const handlePagination = (page) => {
    dispatch(MESSENGER.setPaginationPageMessenger(page));
  };

  const handleRowPerPage = (event) => {
    dispatch(MESSENGER.setPageLimitMessenger(event.target.value));
  };
    ...
    <div className={classes.contentWrapper}>
      <MaterialTable
        ...
        onOrderChange={(orderBy, orderDirection) => {
          dispatch(MESSENGER.onSortFilter(orderBy, orderDirection, !request.order));
        }}
